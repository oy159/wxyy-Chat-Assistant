...
def thread_it(run,):
    ...
    t.start()
"""
tkinter.Button(window, text='开始', command=lambda: thread_it(run, )).place(x=210, y=315)
# 按键停止
tkinter.Button(window, text='结束', command=lambda: window.destroy()).place(x=250, y=315)
# 按键清空
tkinter.Button(window, text='清空', command=lambda: t.delete(0.0, END)).place(x=290, y=315)
window.mainloop()
